...
class FoldersViewModel extends ChangeNotifier {
  FoldersViewModel(this._apiService);
  ...
  Future<void> _loadFolder(String folderId) async {
    ...
    try {
      final detail = await _apiService.getFolderDetail(
        folderId,
        limit: _pageSize,
      );
  ...
  Future<void> loadMoreFolderReels(String folderId) async {
    ...
    try {
      final next = await _apiService.getFolderDetail(
        folderId,
  ...
  Future<void> updateFolder({
    ...
    await _mutate(() async {
      final folder = await _apiService.updateFolder(
        folderId: folderId,
        name: name,
        note: note,
      );
  ...
  Future<void> addReelsToFolder({
    ...
    await _mutate(() async {
      await _apiService.addReelsToFolder(
        folderId: folderId,
        reelIds: reelIds,
  ...
  Future<void> removeReelFromFolder({
    ...
    await _mutate(() async {
      await _apiService.removeReelFromFolder(
        folderId: folderId,
        reelId: reelId,
      );
      final detail = _details[folderId];
